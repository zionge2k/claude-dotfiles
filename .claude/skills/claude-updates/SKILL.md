...
5. **업데이트 브리핑 (마무리)**
   - 문서 저장 후, 대화에서 이번 릴리스의 핵심 내용을 간단히 브리핑할 것
   - 형식:
     - **버전/날짜**: 한 줄
     - **주요 변경 3~5개**: 각 한 줄 bullet (문서의 "핵심 요약" 기반)
     - **내 워크플로우 영향**: 사용자의 스킬/훅/설정에 영향이 있으면 한두 줄, 없으면 "영향 없음" 명시
   - 문서 전체를 다시 붙여넣지 말 것 — 브리핑은 요약본이며, 상세 내용은 저장된 문서 경로를 안내
