...
`vis search`는 **데몬(서버) 실행이 필수**. 데몬이 BGE-M3 모델을 메모리에 상주시켜 검색 응답 0.1~0.5초 (CLI 직접 실행 대비 30~200배 빠름).

```bash
vis serve              # 백그라운드 데몬 시작 (포트 8741)
vis status             # 서버 상태 확인
vis stop               # 서버 중지
```

**자동 시작**: SessionStart 훅이 세션 시작 시 데몬을 자동 시작. 수동 중지/시작은 `vis stop`/`vis serve`로 가능.

**중요**: `vis search` 실행 시 서버가 꺼져 있으면 에러 발생. 다른 명령어(`tag`, `add-related-docs`, `graph` 등)는 데몬 없이도 동작.

**`vis search` 전 데몬 확인 패턴**: search 실행 전 `vis status`로 확인하고, 꺼져 있으면 `vis serve`를 백그라운드로 시작한 후 약 15초 대기 후 검색.
...
| 그래프 시각화 | `vis graph` | "이 문서 그래프 보여줘", "관계 시각화" |
| 서버 시작/관리 | `vis serve/stop/status` | "vis 서버 시작해줘", "서버 상태 확인" |
...
### 그래프 시각화 (graph)
문서 간 관계를 인터랙티브 HTML 그래프로 시각화. Wikilink + 시맨틱 유사도를 결합.
```bash
vis graph "문서명.md"                    # 기본 (depth 1)
vis graph "문서명.md" --depth 2          # 2단계 탐색
vis graph "문서명.md" --depth 3 --top-k 15  # 깊이 3 + 더 많은 노드
vis graph "문서명.md" --no-open -o graph.html  # 브라우저 안 열고 파일만 저장
```

### 서버 관리 (serve/stop/status)
`vis search`를 빠르게 사용하기 위한 데몬 서버 관리.
```bash
vis serve                  # 서버 시작 (포트 8741)
vis serve --port 9000      # 커스텀 포트
vis status                 # 실행 상태 확인
vis stop                   # 서버 중지
```
...
1. **search** - 시맨틱/키워드/하이브리드/ColBERT 검색 ⚠️ 데몬 필수
...
20. **graph** - 문서 관계 그래프 시각화 (PyVis 인터랙티브 HTML)
21. **serve** - 백그라운드 검색 서버 시작
22. **stop** - 검색 서버 중지
23. **status** - 검색 서버 상태 확인
